...
func (ru *Ru864) LinkAdrReq(ChMaskCntl uint8, ChMask lorawan.ChMask, newDataRate uint8, channels *[]c.Channel) (int, []bool, error) {
	...
	for i := ru.GetNbReservedChannels(); i < LenChMask; i++ { //i primi 3 channel sono riservati
	...
	//datarate
	if err = ru.DataRateSupported(newDataRate); err != nil {
		acks[1] = false
	} else if !acks[1] {
