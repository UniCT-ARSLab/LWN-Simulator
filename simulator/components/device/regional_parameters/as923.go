...
func (as *As923) LinkAdrReq(ChMaskCntl uint8, ChMask lorawan.ChMask, newDataRate uint8, channels []c.Channel) (int, []bool, error) {
	...
	for i := as.GetNbReservedChannels(); i < LenChMask; i++ { //i primi 3 channel sono riservati
	...
	//datarate
	if err = as.DataRateSupported(newDataRate); err != nil {
